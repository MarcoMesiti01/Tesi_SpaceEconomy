...
round_stats = df_rt_sum.merge(df_rt_avg, on="Round type", how="left")
chart_data = round_stats.tail(18)

fig, ax_sum = plt.subplots(figsize=(10, 6))
ax_sum.bar(chart_data["Round type"], chart_data["sum"], color="steelblue", label="Total amount invested")
ax_sum.set_xlabel("Round type")
ax_sum.tick_params(axis="x", rotation=90)
ax_sum.set_ylabel("Total amount invested BUSD)")
ax_sum.set_title("Amount invested and average round size per round type")

ax_avg = ax_sum.twinx()
ax_avg.plot(chart_data["Round type"], chart_data["average"], color="darkorange", marker="o", label="Average round size")
...
df_rt_count = (
    df_rt_og.groupby("Round type")
    .size()
    .reset_index(name="round_count")
)
df_rt_count["Round type"].mask(
    df_rt_count["Round type"] == "PROJECT, REAL ESTATE, INFRASTRUCTURE FINANCE",
    other="PROJ, RE, IF",
    inplace=True,
)
df_rt_count.sort_values(inplace=True, by="round_count")

count_chart_data = chart_data[["Round type"]].merge(
    df_rt_count, on="Round type", how="left"
)
...
ax_count.bar(
    count_chart_data["Round type"],
    count_chart_data["round_count"],
    color="slategray",
)
ax_count.set_xlabel("Round type")
ax_count.set_ylabel("Number of rounds")
ax_count.set_title("Number of rounds per round type")
